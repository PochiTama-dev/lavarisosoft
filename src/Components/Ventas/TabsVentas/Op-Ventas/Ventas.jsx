            ...
            <table className='table'>
              <thead>
                <tr>
                 ...
                 <th onClick={() => handleSort('Cliente.nombre')}>Cliente {orderBy === 'Cliente.cuil' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
                  ...
                  {sortedData &&
                    sortedData.map((item, index) => (
                      <tr key={index} className={index % 2 === 0 ? '' : 'row-even'}>
                        ...
                        <td style={{ textAlign: 'left' }}>{`${item.Cliente?.nombre || '-'} ${item.Cliente?.apellido || ''}`.trim()}</td>                     

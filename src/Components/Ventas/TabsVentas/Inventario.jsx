...
const Inventario = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const [stock, tecnicos, reservas] = await Promise.all([
          fetchStockData(),
          fetchTecnicos(),
          fetchReservaData(),
        ]);
  ...
  return (
    <div className="bg-secondary inventario-container">
      ...
      <ul className="d-flex justify-content-around">
        {["Stock", "Stock Camionetas", "Reserva", "Reporte de ventas"].map(
          (name) => (
            <li
              key={name}
              onClick={() => setPestaña(name)}
              className={`pestañasInventario ${
                pestaña === name ? "pestañasInventarioActive" : ""
              }`}
            >
              {name}
            </li>
            ...
            <tbody className="grilla-reserva-body">
              {mergedReservaData.map((reserva, index) => (
                <tr key={index} className={index % 2 === 0 ? "" : "row-even"}>
                  ...
                  <td>
                    {reserva.Repuesto ? reserva.Repuesto.descripcion : "null"}
                  </td>

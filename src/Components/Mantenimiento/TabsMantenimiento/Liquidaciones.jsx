...
const Liquidaciones = () => {
  ...
  const getCobrosOrdenes = async () => {
    ...
    empleadosOrdenes.forEach((empleado) => {
      const datosLiqTecnico = saldosPendientes.filter(
        (liq) => liq.id_tecnico === empleado.empleadoId
      );
      empleado.adelanto = datosLiqTecnico.reduce(
        (acumulador, liq) => acumulador + parseFloat(liq.monto),
        0
      );
    });
  ...
  const handleExportToPDF = () => {
    const filteredData = datosLiquidaciones.filter((liquidacion) => {
      ...
      return (
        (!startDate || fechaOrden >= new Date(startDate)) &&
        (!endDate || fechaOrden <= new Date(endDate))
      );
    });
    ...
    filteredData.forEach((liquidacion) => {
      const liquidacionData = [
        liquidacion.nombre,
        liquidacion.ordenes
          .map((orden) => new Date(orden.created_at).toLocaleDateString())
          .join(", "),
        liquidacion.ordenes
          .reduce(
            (acumulador, orden) =>

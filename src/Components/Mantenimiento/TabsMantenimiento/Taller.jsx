...
const Taller = ({ showHeader }) => {
  ...
  const fetchEmpleados = async () => {
    try {
      ...
      const empleadosFiltrados = empleadosData.filter((tecnico) => {
        if (tecnico.estado === 0 && tecnico.id_rol === 5) {
          return tecnico;
        }
        return false;
      });
  ...
  const handleSave = async (index) => {
    try {
      const tecnicoSeleccionado = empleados.find((empleado) => empleado.id == selectedTecnico);

      if (!tecnicoSeleccionado) {
        throw new Error('Técnico no encontrado');
      }

      const estadoSeleccionado = estados.find((estado) => estado.id == selectedEstado);
  ...
  return (
    <>
      {showHeader && <Header />}
      <div className='taller-ctn' style={showHeader ? { marginTop: 100 } : undefined}>
        ...
        <Table striped hover>
          <thead>
            <tr>
              <th onClick={() => handleSort('equipo')}>Producto {orderBy === 'equipo' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
              <th onClick={() => handleSort('marca')}>Marca {orderBy === 'marca' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
              <th onClick={() => handleSort('modelo')}>Modelo {orderBy === 'modelo' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
              <th onClick={() => handleSort('created_at')}>Fecha {orderBy === 'created_at' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
              <th onClick={() => handleSort('tecD')}>Técnico D. {orderBy === 'tecD' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
              <th onClick={() => handleSort('tecT')}>Técnico T. {orderBy === 'tecT' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
              <th onClick={() => handleSort('numero_orden')}>N° Orden {orderBy === 'numero_orden' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
              <th onClick={() => handleSort('cliente')}>N° Cliente {orderBy === 'cliente' ? orderAsc ? '▲' : '▼' : <span>▼</span>}</th>
          ...
          <tbody>
            {sortedData.map((factura, index) => (
              <tr key={index}>
                ...
                <td>
                  {editIndex === index ? (
                    <select className='form-select' value={selectedTecnico} onChange={handleTecnicoChange}>
                ...
                <td>
                  {editIndex === index ? (
                    <select className='form-select' value={selectedEstado} onChange={handleEstadoChange}>
                      {estados.map((estado) => (
                  ...
                  {editIndex === index ? (
                    <>
                      ...
                      >
                        ...
                        <path d='M9 12.2l2.2 2.2L15.5 9' stroke='white' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
                      </svg>
                      &nbsp;&nbsp;&nbsp;
                      <svg
                        width='24'
                    ...
                    <>
                      {user.tipoRol === 'Jefe de taller' && (
                        <div className='d-flex'>
                          <img src={editar} alt='editar' className='imgEditar' onClick={() => handleEdit(index)} />
                        </div>
                      )}
                    </>
                  )}

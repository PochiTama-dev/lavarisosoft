...
const RemitoLiquidacion = ({
  ...
  const sendToPrinter = async () => {
    ...
    if (cajaSeleccionada) {
      const nuevoMonto =
        cajaSeleccionada.monto - (liqParcial ? liqParcial : tecnico.total);
  ...
  const exportToPDF = () => {
    ...
    doc.text(
      `CUIT/CUIL: ${tecnico.ordenes[0]?.Empleado?.cuil || "N/A"}`,
      10,
      40
    );
    ...
    doc.setFontSize(14);
    doc.text(
      `Porcentaje Técnico: ${tecnico.porcentaje_arreglo * 100}%`,
      10,
      60
    );
    let startY = 70;
    ...
    const tableData = tecnico.ordenes.map((orden) => [
      orden.numero_orden,
    ...
    liqParcial
      ? [
          ...
          doc.text(
            `Debe: ${parseFloat(tecnico.total - liqParcial).toFixed(2)}`,
            10,
            startY + 20
          ),
        ]

...
export const empleadoStockCamioneta = async (idEmpleado) => {
    try {
        const response = await fetch(`${API_URL}/stock/camioneta/empleados/${idEmpleado}`);
        const repuestos = await response.json();
        if (repuestos[0] !== undefined) {
...
export const obtenerRepuestosCamioneta = async (idRepuesto) => {
    try {
        const response = await fetch(`${API_URL}/stock/camioneta/repuestos/${idRepuesto}`);
        const repuestos = await response.json();
        if (repuestos[0] !== undefined) {
...
export const modificarStockCamioneta = async (id, repuesto) => {
    try {
        ...
        const result = await response.json();
        console.log('datosRepuestoJSON: ', JSON.stringify(repuesto))
        console.log('datosRepuesto: ', repuesto)

...
export const clienteCobros = async (idCliente) => {
  try {
    const response = await fetch(`https://lv-back.online/cobros/cliente/${idCliente}`);
    const cobros = await response.json();
    if (cobros) {
...
export const modificarCobro = async (id, cobro) => {
  try {
    const response = await fetch(`https://lv-back.online/cobros/modificar/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(cobro),
    });
...
export const eliminarCobro = async (id) => {
  try {
    const response = await fetch(`https://lv-back.online/cobros/eliminar/${id}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
    });
    const result = await response.json();

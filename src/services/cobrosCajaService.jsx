...
export const clienteCobros = async (idCliente) => {
  try {
    const response = await fetch(`${API_URL}cobros/cliente/${idCliente}`);
    const cobros = await response.json();
    if (cobros) {
      console.log(`Se encontraron ${cobros.length} cobros asociados al cliente con id ${idCliente}`);
      console.log(cobros);
      return cobros;
    } else {
...
export const guardarCobro = async (cobro) => {
  try {
    const response = await fetch(`${API_URL}cobros/guardar`, {
      method: "POST",
    ...
    if (result) {
      console.log("Cobro registrado con éxito!!!");
      console.log(result);
      return true;
    } else {
...
export const modificarCobro = async (id, cobro) => {
  try {
    const response = await fetch(`${API_URL}cobros/modificar/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(cobro),
    });
    const result = await response.json();
    if (result[0] === 1) {
      console.log("Cobro modificado con éxito!!!");
      return true;
    } else {
...
export const eliminarCobro = async (id) => {
  try {
    const response = await fetch(`${API_URL}cobros/eliminar/${id}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    });
    const result = await response.json();

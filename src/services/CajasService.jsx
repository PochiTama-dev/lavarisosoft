...
export const obtenerCaja = async (id) => {
  try {
    ...
    if (caja) {
      console.log(`Se encontró una caja asociado al id ${id}`);
      console.log(caja);
      return caja;
    } else {
...
export const denominacionCaja = async (denominacion) => {
  try {
    const response = await fetch(`https://lv-back.online/cajas/denominacion/${denominacion}`);
    const cajas = await response.json();
    console.log(cajas);
    if (cajas[0] !== undefined) {
...
export const modificarCaja = async (id, caja) => {
  try {
    const response = await fetch(`https://lv-back.online/cajas/modificar/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(caja),
    });
...
export const eliminarCaja = async (id) => {
  try {
    const response = await fetch(`https://lv-back.online/cajas/eliminar/${id}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
    });
    const result = await response.json();
